...
function generateKeys(r) {
  ...
  return a;
    ...
    "#;
        ...
        QuickJsEncryptor { context }
    }

    pub fn encrypt(&self, input: &str, key1: &str, key2: &str, key3: &str) -> String {
        let result = self.context.call_function("strEnc", vec![
            JsValue::String(input.to_string()),
            JsValue::String(key1.to_string()),
            JsValue::String(key2.to_string()),
            JsValue::String(key3.to_string()),
        ]).unwrap();
